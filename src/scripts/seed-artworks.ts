...
// Hardcoded config for the sync script to avoid env loading issues in standalone mode
const client = createClient({
  projectId: "qr1teroj",
  dataset: "production",
  apiVersion: "2023-01-01",
  useCdn: false,
  token: process.env.SANITY_API_WRITE_TOKEN, // Still requires this to be set in the shell or .env
});

if (!process.env.SANITY_API_WRITE_TOKEN) {
  console.error("❌ Error: SANITY_API_WRITE_TOKEN is not set.");
  console.log("Please run: $env:SANITY_API_WRITE_TOKEN='your_token'; npx ts-node src/scripts/seed-artworks.ts");
  process.exit(1);
}
...
async function seed() {
  ...
  for (const artwork of dummyArtworks) {
    try {
      ...
      console.error(`Error syncing ${artwork.title}:`, err);
    }
  }
  
  console.log("Seed process complete. Please manually upload images in the Sanity Studio.");
}
