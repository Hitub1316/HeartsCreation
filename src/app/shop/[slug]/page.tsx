  ...
  return (
    <main className="min-h-screen py-32 px-6 sm:px-12 md:px-24 selection:bg-wine/20 bg-background transition-colors duration-700">
      <div className="max-w-7xl mx-auto">
        ...
        {/* 1. PAINTING IMAGE - THE FOCUS */}
        <div className="mb-16">
          <div className="relative min-h-[50vh] md:min-h-[70vh] flex items-center justify-center overflow-hidden bg-background dark:bg-[#301e20] border whisper-border dark:border-white/5 shadow-sm transition-colors duration-700">
            {imgSource && (
              <img 
                src={imgSource} 
                alt={artwork.title} 
                className="w-auto h-auto max-w-full max-h-[70vh] object-contain shadow-2xl transition-transform duration-1000 hover:scale-[1.01]"
              />
            )}
        ...
        {/* DETAILS SECTION - VERTICAL PRIORITY */}
        <div className="max-w-4xl mx-auto">
          {/* 2. TITLE */}
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif font-light mb-8 leading-tight text-charcoal dark:text-cream transition-colors duration-700">
            {artwork.title}
          </h1>

          {/* 3 & 4. MEDIUM & DIMENSIONS */}
          <div className="flex flex-wrap gap-x-12 gap-y-6 mb-12 py-8 border-y border-charcoal/5 dark:border-white/5 transition-colors duration-700">
            <div className="flex flex-col gap-1">
              <span className="text-[10px] uppercase tracking-[0.2em] text-charcoal/40 dark:text-white/40 font-medium">Medium</span>
              <span className="font-sans font-light text-charcoal/80 dark:text-white/80">{artwork.medium}</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] uppercase tracking-[0.2em] text-charcoal/40 dark:text-white/40 font-medium">Dimensions</span>
              <span className="font-sans font-light text-charcoal/80 dark:text-white/80">{artwork.size}</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] uppercase tracking-[0.2em] text-charcoal/40 dark:text-white/40 font-medium">Authentication</span>
              <span className="font-sans font-light text-charcoal/80 dark:text-white/80">Signed by Artist</span>
            </div>
          </div>

          {/* 5. DESCRIPTION */}
          <div className="text-lg md:text-xl font-serif italic text-charcoal/70 dark:text-cream/70 leading-relaxed mb-12 transition-colors duration-700">
          ...
          <div className="max-w-md">
            ...
            <p className="text-center text-[10px] uppercase tracking-widest text-charcoal/30 dark:text-white/30 transition-colors duration-700">
              Worldwide shipping available. Shipped from {settings?.shippingAddress || "Hyderabad"}.
            </p>
          </div>
        </div>
