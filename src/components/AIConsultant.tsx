    ...
    <>
      ...
      <div className="fixed top-24 left-1/2 -translate-x-1/2 z-40 w-full max-w-xl px-4 pointer-events-none">
        <form 
          ...
          className="pointer-events-auto group relative flex items-center bg-white/40 backdrop-blur-xl border border-charcoal/5 rounded-full px-6 py-3 shadow-sm hover:shadow-md transition-all duration-500"
        >
          <Search className="w-4 h-4 text-charcoal/40 group-focus-within:text-wine transition-colors" />
          <input 
            type="text"
            placeholder="What are you looking for?"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="w-full bg-transparent px-4 text-sm font-sans font-light focus:outline-none text-charcoal"
          />
          <button type="submit" className="text-charcoal/20 hover:text-wine transition-colors">
